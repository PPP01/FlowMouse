...
| `firefox-test` | Firefox-only manifest variant (`background.scripts` + `browser_specific_settings.gecko`). See "Firefox" below. | rebased on the build branch when needed |
...
`firefox-test` swaps the Chrome service-worker manifest for a Firefox one.
